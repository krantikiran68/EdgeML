...
class X86ZeroSkew(X86):

    def __init__(self, outputDir, generateAllFiles, printSwitch, idStr, paramInNativeBitwidth, decls, localDecls, scales, zeros, intvs, cnsts, expTables, globalVars, internalVars, floatConstants, substitutions, demotedVarsOffsets, varsForBitwidth, varLiveIntervals, notScratch, coLocatedVariables):
            ...
            for i in range(len(keys)):
                ...
                if forZeroSkew():
                    if x <= 0:
                        ...
                        if self.varsForBitwidth[arg.idf] <= 16:
                            typeCast = "(uint%d_t*)" % self.varsForBitwidth[arg.idf]
                        else:
                            typeCast = "(int%d_t*)" % self.varsForBitwidth[arg.idf]
