...
class IRBuilder(ASTVisitor):
    ...
    def visitConvolution(self, node: AST.Convolution):
        ...
        prog_conv = IR.Prog(([comment, funcCall, profile] + debugPrint) if forFloat() and self.ddsEnabled else [comment, funcCall])
    ...
    def visitMathExp(self, node: AST.Func):
        ...
        prog_exp = IR.Prog(([cmd0, rangeCheck, funcCall, profile] + debugPrint)if forFloat() and self.ddsEnabled else [cmd0, funcCall])
